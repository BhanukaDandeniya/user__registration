...
import static com.example.userRegistration.Constants.SwaggerConstant.*;
...
public class SwaggerConfiguration {
    ...
    public Docket apiDocket() {
        return new Docket(SWAGGER_2).apiInfo(apiInfo()).forCodeGeneration(true)
                ...
                .securitySchemes(singletonList(apiKey())).select().apis(RequestHandlerSelectors.withClassAnnotation(RestController.class))
                .paths(PathSelectors.regex(SECURE_PATH)).build()
                .tags(new Tag(API_TAG,"All APIs relating to User Registration"));
    }
